...
async function loadHabits() {
    ...
    try {
        ...
        container.innerHTML = `<p class="col-span-full text-center text-sm font-medium text-red-700 bg-red-50 p-4 rounded-2xl border border-red-100">Failed to sync routines. Ensure your database is connected.</p>`;
    }
}
...
async function loadRoutineReport() {
    try {
        ...
        if (data.score === 100) {
            ...
            msgEl.className = "text-sm font-bold text-emerald-700 mt-1";
...
function updateDailyProgress(completed, total) {
    ...
    if (completed === total) {
        ...
        statusTextEl.className = "text-emerald-700 font-bold flex items-center gap-2";
...
async function executeBulkDelete() {
    ...
    const btn = document.getElementById('confirm-delete-habits-button');
